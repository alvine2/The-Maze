...
#include <math.h> // Ensure this header is included
...
void renderFloor(int wallBottomPixel, color_t *texelColor, int x)
{
    ...
    for (y = wallBottomPixel - 1; y < SCREEN_HEIGHT; y++)
    {
        ...
        // Directly handle the modulo operation using integer arithmetic
...
void renderCeil(int wallTopPixel, color_t *texelColor, int x)
{
    ...
    for (y = 0; y < wallTopPixel; y++)
    {
        ...
        // Directly handle the modulo operation using integer arithmetic
        textureOffsetX = (textureOffsetX * texture_width / 40) % texture_width;
        textureOffsetY = (textureOffsetY * texture_height / 40) % texture_height;
